...
At `push`, run `next`, then `gate`, and run the configured push adapter. The built-in Git adapter requires an explicit remote and never force-pushes. Report the actual remote and reviewed commit.
