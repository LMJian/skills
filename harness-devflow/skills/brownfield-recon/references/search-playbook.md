...
- Output paths resolve from the current working directory. Use an absolute path in the selected artifact directory. Repository-local output may live under `.artifacts/` or `.harness/runs/<task>/artifacts/`. `--output -` writes JSON to stdout for a wrapper to capture. Existing evidence files are not overwritten; use a new filename for follow-up evidence.

Use the Harness request's output directory when supplied. Otherwise reuse the existing branch convention, or `.artifacts/brownfield-recon/<branch>/<run>/`. The collector excludes its own `.harness` and `.artifacts` data from source search. Sensitive-file exclusion applies before content access; do not compensate for a skipped secret file by reading it manually.
